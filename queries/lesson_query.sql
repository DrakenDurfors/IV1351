...
SELECT TO_CHAR(TO_DATE(DATE_PART('month', a.date)::text, 'MM'), 'mon') AS month,
       COUNT(DATE_PART('month', a.date))                               AS lesson_amount
FROM (lesson LEFT JOIN schedule_time ON schedule_time.id = lesson.schedule_time_id) AS a
...
-- V.3 Show number of every type of lesson per month for a given year (one row)
SELECT TO_CHAR(TO_DATE(DATE_PART('month', a.date)::text, 'MM'), 'mon') AS month,
       COUNT(DATE_PART('month', a.date))                               AS lessons,
